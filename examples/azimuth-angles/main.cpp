...
int main() {
    ...
    const path dense_model_path{input_path / "audi.ply"};
    auto dense_model = std::make_shared<DenseCloud>(dense_model_path);
    ...
    auto azimuth_data = display_azimuth(model, dense_model, input_image_path, azimuth_angle_data_path, output_path_azimuth);
