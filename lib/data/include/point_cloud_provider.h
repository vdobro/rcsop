...
namespace rcsop::data {
    ...
    private:
        ...
        shared_ptr<vector<IdPoint>> sparse_cloud_points = make_shared<vector<IdPoint>>();
        point_id_t max_sparse_point_id = 0;

        shared_ptr<DenseCloud> dense_mesh = nullptr;
        shared_ptr<vector<IdPoint>> dense_cloud_points = make_shared<vector<IdPoint>>();
    ...
    public:
        ...
        [[nodiscard]] shared_ptr<vector<IdPoint>> get_base_points(size_t take_every_nth = 1) const;

        [[nodiscard]] shared_ptr<vector<IdPoint>> generate_homogenous_cloud(size_t points_per_meter) const;
    };
}
